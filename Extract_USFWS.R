...
##usfws 1 ----
  ...
  for(i in 1:nrow(usfws_1)){
    ...
    if((!sci_name %in% matches_complete$Species.Latin.Name) && (!sci_name %in% matches_complete$short_name)) next  #[CHECK IF NAME IS NOT IN THE MATCHES DATASET]
    num <- num+1
...
##usfws 2 ----
  MGL2_A <- matrix(nrow = 1, ncol = 5)
  colnames(MGL2_A) <-  colnames(usfws_2)
...
#create list of multiples similar ----
  ...
  for(i in 1:nrow(MGL_D)){
    ...
    sci_name <- MGL_D$Scientific_Name[i] #get name for ID
  ...
  for(i in 1:nrow(MGL_D)){
    ...
    MGL_D$season[i] <- 'resident' #assign this ID to a new column
    
    print(paste(sci_name, " season has been added"))
  }
...
#checking existing files and exporting necessary ones
  for(i in 1:nrow(MGL_D)){
